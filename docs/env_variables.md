...
| `TOOL_TIMEOUT` | `20` | Seconds before a tool call times out and returns an error. On timeout, the executor is automatically reset so subsequent calls work without restarting. Lower for fast machines, raise for slow I/O or large files. |
