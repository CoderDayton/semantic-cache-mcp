...
class TestFullCacheLifecycle:
    ...
    async def test_first_read_caches_file(
        ...
        result = await smart_read(semantic_cache, str(test_file))
    ...
    async def test_second_read_returns_cached(
        self, semantic_cache: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        # First read
        result1 = await smart_read(semantic_cache, str(test_file))
    ...
    async def test_modified_file_returns_diff(
        self, semantic_cache: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestStatsAndClear:
    ...
    async def test_stats_empty_cache(self, semantic_cache: SemanticCache) -> None:
    ...
    async def test_stats_after_caching(self, semantic_cache: SemanticCache, temp_dir: Path) -> None:
        """Stats should reflect cached files."""
        # Create and cache files
        for i in range(3):
            f = temp_dir / f"file{i}.txt"
            f.write_text(f"Content for file {i}\n" * 10)
            await smart_read(semantic_cache, str(f))
    ...
    async def test_clear_removes_all(self, semantic_cache: SemanticCache, temp_dir: Path) -> None:
        ...
        # Verify files cached
        assert (await semantic_cache.get_stats())["files_cached"] == 3

        # Clear
        count = await semantic_cache.clear()

        assert count == 3
        assert (await semantic_cache.get_stats())["files_cached"] == 0
...
class TestLargeFiles:
    ...
    async def test_large_file_truncation(
        ...
        result = await smart_read(semantic_cache, str(large_file), max_size=50_000)
    ...
    async def test_large_file_preserves_structure(
        ...
        result = await smart_read(semantic_cache, str(large_py), max_size=10_000)
...
class TestEdgeCases:
    ...
    async def test_nonexistent_file(self, semantic_cache: SemanticCache) -> None:
        """Reading nonexistent file should raise FileNotFoundError."""
        with pytest.raises(FileNotFoundError):
            await smart_read(semantic_cache, "/nonexistent/path/file.txt")

    async def test_empty_file(self, semantic_cache: SemanticCache, temp_dir: Path) -> None:
        """Empty files should be handled gracefully."""
        empty_file = temp_dir / "empty.txt"
        empty_file.write_text("")

        result = await smart_read(semantic_cache, str(empty_file))
    ...
    async def test_unicode_content(self, semantic_cache: SemanticCache, temp_dir: Path) -> None:
        ...
        result = await smart_read(semantic_cache, str(unicode_file))

        assert "" in result.content
        assert "" in result.content
        assert "" in result.content

    async def test_symlink_resolution(self, semantic_cache: SemanticCache, temp_dir: Path) -> None:
        ...
        result = await smart_read(semantic_cache, str(symlink))
...
class TestConcurrentAccess:
    ...
    async def test_multiple_reads_same_file(
        self, semantic_cache: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestMCPToolIntegration:
    ...
    async def test_read_tool_format(self, semantic_cache: SemanticCache, temp_dir: Path) -> None:
    ...
    async def test_stats_json_serializable(
        self, semantic_cache: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestTokenSavings:
    ...
    async def test_unchanged_file_high_savings(
        self, semantic_cache: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_diff_saves_tokens(self, semantic_cache: SemanticCache, temp_dir: Path) -> None:
