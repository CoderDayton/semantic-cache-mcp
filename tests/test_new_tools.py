...
class TestSemanticSearch:
    ...
    async def test_search_empty_cache(self, cache: SemanticCache):
    ...
    async def test_search_respects_k_limit(self, cache: SemanticCache, sample_files: dict):
        ...
        result = await semantic_search(cache, "code", k=2)
    ...
    async def test_search_directory_filter(
        ...
        subdir = temp_dir / "src"
        result = await semantic_search(cache, "code", directory=str(subdir))
...
class TestCompareFiles:
    ...
    async def test_diff_identical_files(self, cache: SemanticCache, temp_dir: Path):
    ...
    async def test_diff_different_files(self, cache: SemanticCache, temp_dir: Path):
    ...
    async def test_diff_uses_cache(self, cache: SemanticCache, sample_files: dict):
        """Diff uses cached content when available."""
        # Pre-cache the files
        await smart_read(cache, str(sample_files["py"]))
        await smart_read(cache, str(sample_files["py2"]))
...
class TestBatchSmartRead:
    ...
    async def test_batch_read_multiple_files(self, cache: SemanticCache, sample_files: dict):
    ...
    async def test_batch_read_respects_token_budget(self, cache: SemanticCache, temp_dir: Path):
        ...
        result = await batch_smart_read(cache, paths, max_total_tokens=100)

        assert result.total_tokens <= 100 or result.files_skipped > 0

    async def test_batch_read_handles_missing_files(self, cache: SemanticCache, temp_dir: Path):
...
class TestGlobWithCacheStatus:
    ...
    async def test_glob_finds_files(self, cache: SemanticCache, sample_files: dict, temp_dir: Path):
        """Glob finds matching files."""
        result = await glob_with_cache_status(cache, "*.py", directory=str(temp_dir))
    ...
    async def test_glob_no_matches(self, cache: SemanticCache, temp_dir: Path):
        """Glob returns empty for no matches."""
        result = await glob_with_cache_status(cache, "*.nonexistent", directory=str(temp_dir))
        assert result.total_matches == 0
        assert result.matches == []

    async def test_glob_cached_only(self, cache: SemanticCache, sample_files: dict, temp_dir: Path):
...
class TestBatchReadOptimizations:
    ...
    async def test_batch_read_diff_mode_false_returns_content_after_cache(
        ...
        result = await batch_smart_read(cache, paths, diff_mode=False)
    ...
    async def test_batch_read_priority_order(self, cache: SemanticCache, sample_files: dict):
        """Priority paths read before sorted remainder."""
        paths = [str(f) for f in sample_files.values()]
        priority_path = str(sample_files["txt"])

        result = await batch_smart_read(cache, paths, priority=[priority_path])

        # First file processed should be the priority file
        assert result.files[0].path == priority_path

    async def test_batch_read_skipped_has_est_tokens(self, cache: SemanticCache, temp_dir: Path):
        ...
        result = await batch_smart_read(cache, paths, max_total_tokens=500)
    ...
    async def test_batch_read_mixed_unchanged_and_new(
        self, cache: SemanticCache, sample_files: dict, temp_dir: Path
    ):
    ...
    async def test_batch_read_priority_does_not_override_budget(
...
class TestSmartBatchEdit:
    ...
    async def test_batch_edit_all_succeed(self, cache: SemanticCache, temp_dir: Path):
    ...
    async def test_batch_edit_partial_success(self, cache: SemanticCache, temp_dir: Path):
    ...
    async def test_batch_edit_all_fail(self, cache: SemanticCache, temp_dir: Path):
    ...
    async def test_batch_edit_dry_run(self, cache: SemanticCache, temp_dir: Path):
    ...
    async def test_batch_edit_uses_cache(self, cache: SemanticCache, temp_dir: Path):
    ...
    async def test_batch_edit_outcomes_have_line_numbers(
        self, cache: SemanticCache, temp_dir: Path
    ):
    ...
    async def test_batch_edit_reports_errors(self, cache: SemanticCache, temp_dir: Path):
    ...
    async def test_batch_edit_preserves_line_positions(self, cache: SemanticCache, temp_dir: Path):
