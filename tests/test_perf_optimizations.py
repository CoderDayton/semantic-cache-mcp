"""Tests for performance optimizations: gather, no double-fetch, no needless refresh."""
...
class TestBatchReadGather:
    ...
    async def test_batch_read_reuses_cache_entries(self, tmp_path: Path) -> None:
        """The batch path should reuse entries from the gather, not re-fetch."""
        cache = _make_cache(tmp_path)
        ...
        with patch.object(SemanticCache, "get", counting_get):
