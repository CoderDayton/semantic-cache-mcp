...
class TestSmartWriteNewFile:
    ...
    async def test_write_new_file(
        ...
        result = await smart_write(semantic_cache_no_embeddings, str(file_path), content)
    ...
    async def test_write_no_create_parents_fails(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestSmartWriteOverwrite:
    ...
    async def test_write_overwrites_existing(
        ...
        result = await smart_write(semantic_cache_no_embeddings, str(file_path), "new content\n")
    ...
    async def test_write_updates_cache(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        """Write updates cache for subsequent reads."""
        file_path = temp_dir / "cached.txt"

        await smart_write(semantic_cache_no_embeddings, str(file_path), "first\n")
        await smart_write(semantic_cache_no_embeddings, str(file_path), "second\n")
...
class TestSmartWriteDryRun:
    ...
    async def test_write_dry_run_no_write(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        result = await smart_write(
            semantic_cache_no_embeddings, str(file_path), "content", dry_run=True
        )

        assert result.created is True
        assert not file_path.exists()

    async def test_write_dry_run_shows_diff(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        result = await smart_write(
            semantic_cache_no_embeddings, str(file_path), "new\n", dry_run=True
        )
...
class TestSmartWriteEdgeCases:
    ...
    async def test_write_binary_rejection(
        self, semantic_cache_no_embeddings: SemanticCache, binary_file: Path
    ) -> None:
        """Cannot overwrite binary file with text."""
        with pytest.raises(ValueError, match="Binary file"):
            await smart_write(semantic_cache_no_embeddings, str(binary_file), "text")

    async def test_write_returns_hash(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestSmartEditBasic:
    ...
    async def test_edit_single_match(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        result = await smart_edit(semantic_cache_no_embeddings, str(file_path), "World", "Universe")

        assert result.matches_found == 1
        assert result.replacements_made == 1
        assert file_path.read_text() == "Hello Universe\n"

    async def test_edit_returns_line_numbers(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        result = await smart_edit(semantic_cache_no_embeddings, str(file_path), "foo", "bar")

        assert result.line_numbers == [2]

    async def test_edit_returns_diff(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        result = await smart_edit(semantic_cache_no_embeddings, str(file_path), "old", "new")
...
class TestSmartEditMultipleMatches:
    ...
    async def test_edit_multiple_matches_fails(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        with pytest.raises(ValueError, match="found 2 times"):
            await smart_edit(semantic_cache_no_embeddings, str(file_path), "foo", "baz")
...
class TestSmartEditErrors:
    ...
    async def test_edit_not_found_error(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        with pytest.raises(ValueError, match="not found"):
            await smart_edit(semantic_cache_no_embeddings, str(file_path), "missing", "replacement")
    ...
    async def test_edit_file_not_found(
        ...
        with pytest.raises(FileNotFoundError):
            await smart_edit(semantic_cache_no_embeddings, str(file_path), "a", "b")
    ...
    async def test_edit_identical_strings(
        ...
        with pytest.raises(ValueError, match="identical"):
            await smart_edit(semantic_cache_no_embeddings, str(file_path), "same", "same")

    async def test_edit_binary_rejection(
        self, semantic_cache_no_embeddings: SemanticCache, binary_file: Path
    ) -> None:
...
class TestSmartEditCache:
    ...
    async def test_edit_uses_cache(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        result = await smart_edit(
            semantic_cache_no_embeddings, str(file_path), "cached", "modified"
        )

        assert result.from_cache is True

    async def test_edit_updates_cache(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        await smart_edit(semantic_cache_no_embeddings, str(file_path), "original", "modified")
...
class TestSmartEditDryRun:
    ...
    async def test_edit_dry_run_no_write(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestTokenSavings:
    ...
    async def test_write_token_savings(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        result = await smart_write(semantic_cache_no_embeddings, str(file_path), "".join(new_lines))

        # Diff should be much smaller than full content
        assert result.tokens_saved > 0

    async def test_edit_token_savings(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestAutoFormat:
    ...
    async def test_write_auto_format_no_formatter(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_write_auto_format_false_default(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_edit_auto_format_no_formatter(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestSuppressLargeDiff:
    ...
    async def test_small_diff_passes_through(self) -> None:
        """Small diffs are returned unchanged."""
        diff = "@@ -1,3 +1,3 @@\n-old\n+new\n context\n"
        result = _suppress_large_diff(diff, full_tokens=500)
        assert result == diff

    async def test_large_diff_returns_summary(self) -> None:
...
class TestSmartWriteAppend:
    ...
    async def test_append_creates_new_file(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_append_concatenates_content(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        ...
        await smart_write(semantic_cache_no_embeddings, str(file_path), "chunk2\n", append=True)

        assert file_path.read_text() == "chunk1\nchunk2\n"

    async def test_append_multiple_chunks(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
        """Multiple appends build up file incrementally."""
        file_path = temp_dir / "append_multi.txt"

        await smart_write(semantic_cache_no_embeddings, str(file_path), "line1\n")
        await smart_write(semantic_cache_no_embeddings, str(file_path), "line2\n", append=True)
        await smart_write(semantic_cache_no_embeddings, str(file_path), "line3\n", append=True)

        assert file_path.read_text() == "line1\nline2\nline3\n"

    async def test_append_returns_diff(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestExtractLineRange:
    ...
    async def test_multi_line(self) -> None:
        content = "line1\nline2\nline3\nline4\n"
        sub, cs, ce = _extract_line_range(content, 2, 3)
        assert sub == "line2\nline3\n"
        assert content[:cs] + "replaced\n" + content[ce:] == "line1\nreplaced\nline4\n"

    async def test_full_file(self) -> None:
        content = "a\nb\n"
    ...
    async def test_start_line_zero_raises(self) -> None:
    ...
    async def test_start_exceeds_total_raises(self) -> None:
        with pytest.raises(ValueError, match="start_line.*exceeds total"):
            _extract_line_range("a\n", 3, 3)

    async def test_end_exceeds_total_raises(self) -> None:
...
class TestSmartEditLineRange:
    ...
    async def test_mode_b_finds_match_in_range(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_b_not_found_outside_range(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_b_reports_absolute_line_numbers(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_b_replace_all_within_range(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_b_multiple_matches_without_replace_all_fails(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_c_replaces_line_range(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_c_single_line(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_c_multiline_new_string(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_c_returns_diff(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_c_reject_replace_all(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_old_string_none_without_line_range_raises(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_only_start_line_raises(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_out_of_bounds_raises(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_c_no_trailing_newline(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_c_no_trailing_newline_multiline(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
...
class TestSmartBatchEditLineRange:
    ...
    async def test_mode_c_in_batch(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mode_b_in_batch(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_mixed_modes_in_batch(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_invalid_range_becomes_failure(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
    ...
    async def test_batch_mode_c_no_trailing_newline(
        self, semantic_cache_no_embeddings: SemanticCache, temp_dir: Path
    ) -> None:
